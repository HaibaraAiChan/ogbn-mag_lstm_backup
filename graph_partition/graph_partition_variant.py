...
def updateRed_group(locked_nodes, cur_rate, alpha,ideal_part_in_size):
    ...
    print('the number of node to move A-> Bis :', len(nid_to_move))
    print()
    if len(nid_to_move)>0:
        for i in nid_to_move:
            locked_nodes[i]=True
        if not move_group_nids_balance_redundancy_check(bit_dict, side, nid_to_move,alpha, cur_rate,ideal_part_in_size):
    ...
    if len(ready_to_move)>0:    
        for i in ready_to_move:
            locked_nodes[i]=True
        if not move_group_nids_balance_redundancy_check(bit_dict, side, ready_to_move,alpha, cur_rate,ideal_part_in_size):
...
def walk_terminate_1(g,red_rate, args,ideal_part_in_size):
    ...
    for i in range(steps_):
        tt=time.time()
        updateRed_group(locked_nodes,red_rate,args.alpha,ideal_part_in_size)
        tt_e=time.time()
        if i % 1==0:
            print('\t\t\tone update redundancy\t', tt_e-tt)	
            print('\t\t  ---------------------------------------------------- n',i)
        totalSteps+=1
        flag, len_A_part,len_B_part = balance_checking(args.alpha)
